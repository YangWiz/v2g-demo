...
#define dump_flag(v, b)                                                        \
  Tree->m_Definition.AppendFormat(L" " L#b L"= %d\r\n", v &b ? 1 : 0)
#define dump_enum(n, e)                                                        \
  Tree->m_Definition.AppendFormat(L#n L"= %u (%s)\r\n", n, enumerate(n, e))
...
#define dump_link(f, n)                                                        \
  {                                                                            \
    ...
    if (0 == getLink(n)) {                                                     \
      t->m_Definition = _T("-NIL-");                                           \
    } else {                                                                   \
      m4Block *b = f.LoadLink(*this, n);                                       \
      if (b == NULL)                                                           \
        t->m_Definition = _T("?FAILED?");                                      \
      else {                                                                   \
        b->dump(f, t);                                                         \
        delete b;                                                              \
      }                                                                        \
...
#define dump_mem(n, f)                                                         \
  {                                                                            \
    sprintf(szGlobalBuffer, #n "=" f "\r\n", n);                               \
    strcat(Tree->m_Definition, szGlobalBuffer);                                \
  }
#define dump_var(n, v)                                                         \
  {                                                                            \
    sprintf(szGlobalBuffer, n "\r\n", v);                                      \
    strcat(Tree->m_Definition, szGlobalBuffer);                                \
  }
#define dump_flag(v, b)                                                        \
  {                                                                            \
    sprintf(szGlobalBuffer, " " #b "= %d\r\n", v &b ? 1 : 0);                  \
    strcat(Tree->m_Definition, szGlobalBuffer);                                \
  }
#define dump_enum(n, e)                                                        \
  {                                                                            \
    sprintf(szGlobalBuffer, #n "= %u (%s)\r\n", n, enumerate(n, e));           \
    strcat(Tree->m_Definition, szGlobalBuffer);                                \
  }
...
void M4HDBlock::dump(MDF4File &f, m4TreeItem *Tree) {
  ...
  Tree->m_Definition.AppendFormat(
      _T(" (converted) %04d %02d. %02d. %02d:%02d:%02d.%d \r\n"), st.wYear,
...
void M4EVBlock::dump(MDF4File &f, m4TreeItem *Tree) {
  ...
  static const m4Enumeration enum_ev_sync_type[] = {
      _e(EV_S_TIME)     // calculated synchronization value represents time in
                        // seconds
      _e(EV_S_ANGLE)    // calculated synchronization value represents angle in
                        // radians
      _e(EV_S_DISTANCE) // calculated synchronization value represents distance
                        // in meter
      _e(EV_S_INDEX) // calculated synchronization value represents zero-based
                     // record index
      {NULL, 0}};
  ...
  static const m4Enumeration enum_ev_cause[] = {
      _e(EV_C_OTHER)  // OTHER cause of event is not known or does not fit into
                      // given categories.
      _e(EV_C_ERROR)  // ERROR event was caused by some error.
      _e(EV_C_TOOL)   // TOOL event was caused by tool-internal condition, e.g.
                      // trigger condition or re-configuration.
      _e(EV_C_SCRIPT) // SCRIPT event was caused by a scripting command.
      _e(EV_C_USER)   // USER event was caused directly by user, e.g. user input
                      // or some other interaction with GUI.
      {NULL, 0}};
  m4Block::dump(f, Tree);
  // memory
...
void M4SIBlock::dump(MDF4File &f, m4TreeItem *Tree) {
  static const m4Enumeration enum_si_type[] = {
      _e(SI_T_OTHER)  // OTHER source type does not fit into given categories or
                      // is unknown
      ...
      _e(SI_T_TOOL)   // TOOL source is a software tool (e.g. for tool generated
                      // signals/events)
      _e(SI_T_USER)   //  USER source is a user interaction/input (e.g. for user
                      //  generated events)
      {NULL, 0}};
  static const m4Enumeration enum_si_bus_type[] = {
      _e(SI_BUS_NONE)  // NONE no bus
      _e(SI_BUS_OTHER) // OTHER bus type does not fit into given categories or
                       // is unknown
      _e(SI_BUS_CAN)   // CAN
      _e(SI_BUS_LIN)   // LIN
      _e(SI_BUS_MOST)  // MOST
...
void M4CCBlock::dump(MDF4File &f, m4TreeItem *Tree) {
  static const m4Enumeration enum_cc_type[] = {
      _e(CC_T_IDENTITY) // 1:1 conversion (in this case, the CCBLOCK simply can
                        // be omitted)
...
void M4CHBlock::dump(MDF4File &f, m4TreeItem *Tree) {
  static const m4Enumeration enum_ch_type[] = {
      _e(CH_T_GROUP) _e(CH_T_FUNCTION) _e(CH_T_STRUCTURE) _e(CH_T_MAPLIST)
          _e(CH_T_PARAMETER) _e(CH_T_RESULT) _e(CH_T_LOCAL) _e(CH_T_CALDEF)
              _e(CH_T_CALREF){NULL, 0}};
  m4Block::dump(f, Tree);
  // memory
...
void M4CABlock::dump(MDF4File &f, m4TreeItem *Tree) {
  static const m4Enumeration enum_ca_type[] = {_e(CA_T_ARRAY) _e(CA_T_AXIS)
                                                   _e(CA_T_LOOKUP){NULL, 0}};
  static const m4Enumeration enum_ca_storage[] = {
      _e(CA_S_CN_TEMPLATE) _e(CA_S_CG_TEMPLATE) _e(CA_S_DG_TEMPLATE){NULL, 0}};
  m4Block::dump(f, Tree);
  // memory
