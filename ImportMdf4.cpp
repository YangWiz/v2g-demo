...
BOOL CMdf4DataGroup::GetRecord(M4CGBlock *cg, M_BYTE *ptr, M_UINT64 ix1, M_UINT64 ix2)
{
	...
	// Use direct data stream for small reads
	...
	// Lazy initialization of m_pRF as a class member that persists
	// rather than recreating it every time
	if (m_pRF == NULL) {
		m_pRF = new CReadFileMF4;
		// Choose buffer size based on available memory and typical access patterns
		size_t optimalBatchSize = std::min<size_t>(16384, n);
		if (!m_pRF->Open(m_dg, RecLen, optimalBatchSize)) {
			delete m_pRF;
			m_pRF = NULL;
			return FALSE;
		}
...
bool CMdf4FileImport::displayByName(const std::string &name) {
	for (const auto dg_ptr : this->m_vDataGroups) {
		...
		while (cn) {
			if (this->findChannelByName(cn, name)) {
				this->DisplayData(pGroup, dg, cg, cn);
				return true;
			}
...
bool CMdf4FileImport::getValueVec(CMdf4DataGroup* pGroup, M4DGBlock* dg,
				   M4CGBlock* cg, M4CNBlock* cn, std::vector<double> &result) {
	...
	constexpr int BATCH_SIZE = 4096;  // Adjust based on your system characteristics
...
bool CMdf4FileImport::findChannelByName(M4CNBlock *cn, const std::string& name) {
...
BOOL CMdf4FileImport::ImportFile(const char *szFileName)
{
	...
	M_UINT32 nGrp=0;
	M4CGBlock *cg;
	auto *dg = dynamic_cast<M4DGBlock *>(m_m4.LoadLink(*pHdr, M4HDBlock::hd_dg_first));
	while (dg)
	{
		cg = dynamic_cast<M4CGBlock *>(m_m4.LoadLink(*dg, M4DGBlock::dg_cg_first));
		while (cg)
		{
			auto *p = new CMdf4DataGroup;
			...
			cg = dynamic_cast<M4CGBlock *>(LoadLink(cg, M4CGBlock::cg_cg_next)); // would be unsorted -> not supported
		}
