...
public:
  ...
  M_UINT32 Read(M_UINT32 dwSize, void *pBuffer) {
    ...
    if (m_iBufCtr++ == 0) {
      bResult =
          m_pDataStream->Read(m_dwBufferSize, m_pBuffer) == m_dwBufferSize;
...
public:
  ...
  BOOL GetRawValueFromRecord(M4CGBlock *cg, M4CNBlock *cn, M_BYTE *pRecord,
                             double *pValue);
  double GetValueFromRecord(M_BYTE *dptr, int sbit, int nbit, int nbytes,
                            BOOL bInteger, BOOL bSigned, BOOL bMotorola,
                            BOOL bFloat);
};
...
public:
  ...
  bool findChannelByName(M4CNBlock *cn, const std::string &name);
  ...
  void DisplayData(CMdf4DataGroup *pGroup, M4DGBlock *dg, M4CGBlock *cg,
                   M4CNBlock *cn);
