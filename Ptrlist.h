...
public:
  ...
  public:
    ...
    T *operator->() const { return (_Ptr ? _Ptr->_Value : NULL); }
    iterator &operator++() {
  ...
  T *push(T *_X, bool _O) {
    if (!_Size++)
      _Head = new _Node(_X, _O);
    else
      _Head = new _Node(_X, _Head, _O);
    return _X;
  }
  ...
  void remove(T *_X, bool _D = false) { remove(find(_X), _D); }
  void pop() { remove(top()); }
...
// TEMPLATE CLASS ptrlist
template <class T> class DbtObjPtrList {
protected:
  class _Node {
  public:
    _Node *_Next, *_Prev;
